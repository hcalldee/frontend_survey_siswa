...
// BASE_URL dipakai sebagai prefix endpoint (di sini kita pakai relative via proxy server app_user)
const BASE_URL = "";
...
function renderTabelRating(parsedData, pengajarStatus) {
  ...
  parsedData.forEach((kategoriObj, kategoriIdx) => {
    ...
    let colspan = isKategoriII ? pengajarStatus.length + 2 : 2 + 99;
    const $komentarRow = $(
      `
  ...
  if ($(this).html() === 'Simpan') {
    ...
    $('.jawaban-rating').each(function () {
      ...
      if ($el.is('select')) {
        ...
        // jangan trim, biar separator <br> aman
        const komentar = $el.val() || '';
...
$(function () {
  ...
  // detail pelatihan + kategori render
